...
%% 初始化
function main(index,closesize)
%% 初始化：图片路径，闭操作窗口大小,加载变换矩阵
disp("正在处理"+index+"......")
...
format = ".jpg";
loadt1 = load(index+".mat");
t1 = loadt1.t1;
imgtrans(path,optical_name,sar_name,infrared_name,format,closesize,t1)
end



%% 变换与保存
function imgtrans(path,optical_name,sar_name,infrared_name,format,closesize,t1)
%% 读入图片
...
saveas(gcf,path+optical_name+sar_name+"blend.jpg")
close
toc
...
figure, imshow(imresize(overlap_mask,0.1))
saveas(gcf,path+optical_name+sar_name+"mask.jpg")
close
toc
